function [fig,timeVector,y,countryName,order] = covid_plot_who(criterionDays,large,cum,mustHave)
if ~exist('large','var')
    large = true;
end
if ~exist('cum','var')
...
[isx,idx] = ismember(missing(:,1),whoData.Country);
whoData.Country(idx) = missing(:,2);
whoData(~ismember(whoData.Country,popw.Country_Other),:) = [];
[~,idx1] = ismember(whoData.Country,popw.Country_Other);
popw = popw(idx1,:);
date
if large
    small = popw.Population < 1000000;
    whoData(small,:) = [];
end
