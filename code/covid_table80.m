...
tv = readtable('tmp.csv');
dateAll = cellfun(@(x) datetime(x(1:10)),tv{:,1});
date = unique(dateAll);
for ii = 1:length(date)
    row = find(dateAll == date(ii));
    row = row(end);
    
    doseIover80(ii,1) = sum(tv{row,[24,27]})./sum(tv{row,[23,26]});
    doseIIover80(ii,1) = sum(tv{row,[25,28]})./sum(tv{row,[23,26]});
    doseIbelow80(ii,1) = sum(tv{row,[3:3:21]})./sum(tv{row,[2:3:20]});
    doseIIbelow80(ii,1) = sum(tv{row,[4:3:22]})./sum(tv{row,[2:3:20]});
end
vacc = table(date,doseIbelow80,doseIover80,doseIIbelow80,doseIIover80);
covid_age_subplot1
ir = ismember(date,vacc.date);
vacc.deaths_below80 = round(sum(yy(ir,1:end-2),2));
vacc.deaths_over80 = round(sum(yy(ir,end-1:end),2));
