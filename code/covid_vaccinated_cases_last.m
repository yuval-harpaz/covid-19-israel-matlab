...
%     vaccA.cum1(row) = cumsum(vaccA.first_dose(row));
...
lastweek.dose1_cases = floor(sum(cellfun(@str2num, strrep(lastweek{:,5:8},'<5','2.5')),2));
lastweek.dose2_cases = floor(sum(cellfun(@str2num, strrep(lastweek{:,9:11},'<5','2.5')),2));

try
    lastweek.unvaccinated_cases = floor(cellfun(@str2num, strrep(lastweek{:,12},'<5','2.5'))) + ...
        floor(cellfun(@str2num, strrep(lastweek{:,4},'<5','2.5')));
catch
    lastweek.unvaccinated_cases = lastweek{:,12} + ...
        floor(cellfun(@str2num, strrep(lastweek{:,4},'<5','2.5')));
end

midDate = datetime(weekk{end}(1:10))+3;
row = find(datetime(cellfun(@(x) x(1:10),vac.Date,'UniformOutput',false)) == midDate,1,'last');
% vaccX = vaccA(ismember(vaccA.date,midDate-7),:);
% [~,order] = sort(vaccX.age_group);
% vaccX = vaccX(order,:);
lastweek.dose1_population = vac{row,3:3:end}';
lastweek.dose2_population = vac{row,4:3:end}'
lastweek.population =  [2777000*1.02+591000;vac{row,5:3:end}'];
lastweek = lastweek(:,[2,3,13:end]);
VE = (lastweek.dose2_cases./lastweek.dose2_population)./...
    (lastweek.unvaccinated_cases./(lastweek.population-lastweek.dose1_population));
VE = 100-100*VE;
table(lastweek.Age_group,VE)
writetable(lastweek,['~/',weekk{end},'.csv'],'Delimiter',',','WriteVariableNames',true)
% %%
% % iAge = 6:9;
% % vaccX = vaccA(ismember(vaccA.age_group,ages(iAge)),:);
% population = [2777000*1.02+591000;1318000;1206000;1111000;875000;749000;531000;308000-51500;51500];
