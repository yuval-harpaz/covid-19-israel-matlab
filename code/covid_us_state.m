function [fig,date,y,countryHeb,order,ht] = covid_us_state(state)
if ~exist('state','var')
    state = 'Florida';
end
if ~exist('large','var')
...
nCountries = 20;
% type = 'deaths';
% options = weboptions('ArrayFormat','csv');
!wget -O tmp.csv https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_US.csv
deathsT = readtable('tmp.csv');
date = datetime([deathsT{1,13:end}])';
date = date(2:end);
deaths = diff(sum(cellfun(@str2num,deathsT{ismember(deathsT.Var8,'US') & ismember(deathsT.Var7,state),13:end}))');
...
date = datetime(dataMatrix(1,5:end))';
us = dataMatrix(find(ismember(dataMatrix(:,2),'US')),:)
