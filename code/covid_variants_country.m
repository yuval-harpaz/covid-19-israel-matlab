function covid_variants_country(country)
if ~exist('country','var')
    country = [];
end
if isempty(country)
    country = 'Israel';
...
json = jsondecode(txt);
countriesU = unique({json.regions.distributions(:).country}');
cstr = join(countriesU',', ');
if ~ismember(country,countriesU)
    disp(countriesU')
    error(['country ',country,' not in json.'])
end
iCountry = find(ismember({json.regions.distributions(:).country}',country));
data = struct2table(json.regions.distributions(iCountry).distribution);
week = cellfun(@datetime, data.week);

variant = fieldnames(data.stand_estimated_cases);
tt = struct2table(data.stand_estimated_cases(:));
...
cases = diff(mergedData{ismember(mergedData(:,1),country),2});
cases(cases < 0) = nan;
% dateH = week+5;
!wget -O tmp.csv https://raw.githubusercontent.com/owid/covid-19-data/9529ccf8b91b87b662e8581fcf4659252bbbdf84/scripts/input/un/population_latest.csv
pop = readtable('tmp.csv');
pop1 = pop.population(ismember(pop.entity, country));
figure;
plot(week+7,round(sum(tt{:,:},2)),'r')
hold on
plot(timeVector(2:end),round(movsum(cases,[7 6],'omitnan'))/pop1*10^6,'k')
grid on
box off
title(country)
legend('sum(variants)','14 days rolling sum of cases','location','northwest')
ax = gca;
ax.YRuler.Exponent = 0;
ax.YAxis.TickLabelFormat = '%,.0g';
xtickangle(30)
xlim([datetime(2020,1,1) datetime('today')+7])
set(gcf,'Color','w')
ylabel('cases per 10M')
