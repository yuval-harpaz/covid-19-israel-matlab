...
app.layout = html.Div([
    html.Div([
        html.Div([
            ...
            # html.A(' some links here: '), html.A('@yuvharpaz.', href="https://twitter.com/yuvharpaz", target='_blank'),html.A(' '),
            # html.Div([
            #     html.A('A button to download stuff  '),
            #     html.Button("Download", id="btn-download-txt"),
            #     dcc.Download(id="download-text")]
            # ),
            # html.Br(), html.A('Another link: '),
            # html.A('South Africa',
            #        href="https://en.wikipedia.org/wiki/Apartheid",
            #        target='_blank'),
            html.Br(), html.Br()
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Row([html.P(text, id='text', dir='rtl'), html.Br(), html.Br()]) for text in texts
            ])  # ,lg=9, align='start'
        ], justify='end'),
...
def update_text(*args):
    ...
    for iis in range(len(issue)):
        if len(reasons[iis]) > 0:
            optext.append(reasons[iis][:-2])
            optext.append(issue[iis]['description']+'\n' + \
                          issue[iis]['example']+'\n' + \
                          issue[iis]['ref'])
