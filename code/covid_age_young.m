function covid_age_young
% [posDash, dateD] = get_dashboard;
[pos, tests, date, ages] = getTimnaY;
...
for ii = 1:2
    ...
    if ii == 1
        ...
        hh = plot(date-3,tests);
        title('weekly tests by age')
    end
    set(gca,'FontSize',13,'Xtick',datetime(2020,1:50,1))
    grid on
    ax = gca;
    ax.YRuler.Exponent = 0;
    xlim([date(1)-3,datetime('today')])
    xtickformat('MMM')
    legend(ages,...
...
a=1;
% pos = [pos(:,1),sum(pos(:,2:5),2),sum(pos(:,6:9),2),sum(pos(:,10:13),2),pos(:,14)];
