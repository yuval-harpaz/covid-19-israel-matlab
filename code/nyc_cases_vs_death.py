...
fig.add_trace(go.Scatter(x=date[:-clipEnd]+np.timedelta64(17), y=movmean(np.asarray(data['HOSPITALIZED_COUNT'][:-clipEnd])/nrm[1], 7),
                            name='hosp', line_color='red', line_width=3))
fig.add_trace(go.Scatter(x=date[:-clipEnd]+np.timedelta64(20), y=movmean(np.asarray(data['CASE_COUNT'][:-clipEnd])/nrm[2], 7),
                            name='cases', line_color='cyan', line_width=3))
fig.add_trace(go.Scatter(x=dateW+np.timedelta64(15), y=cases65/nrm[3],
                            name='cases 65+', line_color='blue', line_width=3))
