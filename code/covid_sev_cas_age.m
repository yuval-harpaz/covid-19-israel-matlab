...
[posC, dateWC, agesC] = get_dashboard_cases;
[posS, dateWS, agesS] = get_dashboard_severe;
if dateWC(end) ~= dateWS(end)
    error('severe and cases end in a different date')
end
posC = posC(ismember(dateWC,dateWS),:);

ratio = nansum(posS)./nansum(posC);
figure;
bar([ratio(:,1:10)',ratio(:,11:20)'])
set(gca,'YScale','log','FontSize',13,'XTickLabel',agesC)
grid on

figure;
bar(1./[ratio(:,1:10)',ratio(:,11:20)'])
set(gca,'YScale','log','FontSize',13,'XTickLabel',agesC)
grid on

cpm30 = round(10^6.*[sum(nansum(posS(:,4:10))),sum(nansum(posS(:,14:20)))]./[sum(nansum(posC(:,4:10))),sum(nansum(posC(:,14:20)))]);
