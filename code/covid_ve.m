...
for ii = 1:month(datetime('today'))
    ...
    ve2(ii,1) = 100*(1-(nanmedian(cases.verified_vaccinated_normalized(cOld & im))...
        ./nanmean(cases.verified_not_vaccinated_normalized(cOld & im))));
    ve2(ii,2) = 100*(1-(mean(severe.serious_vaccinated_normalized(cOld & im))...
        ./mean(severe.serious_not_vaccinated_normalized(cOld & im))));
    ve2(ii,3) = 100*(1-(mean(deathsm.death_vaccinated_normalized(cOld & im))...
        ./mean(deathsm.death_not_vaccinated_normalized(cOld & im))));
    if ii < 8
        ve3(ii,1:3) = nan;
    else
        ve3(ii,1) = 100*(1-(nanmean(cases.verified_boost_vaccinated_normalized(cOld & im))...
            ./nanmean(cases.verified_not_vaccinated_normalized(cOld & im))));
        ve3(ii,2) = 100*(1-(mean(severe.serious_boost_vaccinated_normalized(cOld & im))...
            ./mean(severe.serious_not_vaccinated_normalized(cOld & im))));
        ve3(ii,3) = 100*(1-(mean(deathsm.death_amount_boost_vaccinated_normalized(cOld & im))...
            ./mean(deathsm.death_not_vaccinated_normalized(cOld & im))));
    end
end
ve2(5:6,:) = nan;
...
title('VE for 60+ by month, dose II')
set(gcf,'Color','w')
box off

subplot(2,1,2)
hb3 = bar(mnu,ve3,'EdgeColor','none');
...
xtickangle(0)
set(gca,'ygrid','on','YTick',0:10:100)
title('VE for 60+ by month, dose III')
set(gcf,'Color','w')
box off
%%
