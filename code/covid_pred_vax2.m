function tot = covid_pred_vax2(vaxPerDay,dayEffect)
...
subplot(2,1,1)
iStart = 275;
hx(1) = plot(tests.date(1):tests.date(1)+length(x)-1,x,'k');
hold on;
...
legend(hx,'מאומתים בני 60+ (בדיקה ראשונה)','מנבא תמותה הכולל חיסון מבוגרים','מנבא תמותה הכולל סגר','location','northwest')
...
hb2(3) = plot(tests.date(1)+iStart-1:tests.date(1)+length(predSeger)-1,predSeger(iStart:end)./10+add,'r');
legend(hb2,'תמותה','ניבוי תמותה הכולל חיסון מבוגרים','ניבוי תמותה הכולל סגר','location','northwest')
grid on
xlim([datetime(2020,6,15) datetime(2021,4,1)]);
title('הניבוי')
ylabel('נפטרים')
